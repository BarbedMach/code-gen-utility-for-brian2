...
class Array1D(GlobalVar, IDefinition, IArray):
    def __init__(self, name: str, size: int, value, addr: int):
    ...
    def generateAllocationBlock(self):
        ...
        if platform.system() == 'Windows':
            ...
            result += f'SIZE_T {self.name}_size = sizeof(double) * {self.size};\n'
    ...
    def generateInitializerFunction(self):
        ...
        result += f'for (int i = 0; i < {self.size}; ++i) {{\n'
        result += f'{self.name}[i] = {self.value};\n'
        result += '}\n'
        result += 'return 0;\n'
        result += '}\n'
    ...
    def generateDeleteCall(self):
        ...
        if platform.system() == 'Windows':
            ...
            result += f"munmap((void*)d_p_{self.name}, {self.size});\n"
