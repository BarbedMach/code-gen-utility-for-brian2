...
WTET_stim = TimedArray(wtet_pattern * Hz, 0.2 * second)

late_STET_stim = TimedArray(np.concatenate((np.zeros(3600), stet_pattern)) * Hz, 1 * second)
late_SLFS_stim = TimedArray(np.concatenate((np.zeros(int(3600 * second / (50 * ms))), slfs_pattern)) * Hz, 50 * ms)
late_WLFS_stim = TimedArray(np.concatenate((np.zeros(int(3600 * second / (900 * second))), wlfs_pattern)) * Hz, 900 * second)
late_WTET_stim = TimedArray(np.concatenate((np.zeros(int(3600 * second / (0.2 * second))), wtet_pattern)) * Hz, 0.2 * second)


STIM_PROTOCOL_S1 = 'WTET'
STIM_PROTOCOL_S2 = 'late_SLFS'
